...
CGameObject::CGameObject(const CGameObject& Obj)	:
	...
	m_Size(Obj.m_Size),
	m_Pivot(Obj.m_Pivot),
	m_OriginPivot(Obj.m_OriginPivot),
...
void CGameObject::Update(float DeltaTime)
{
	if (!m_RenderState)
	{
		...
		m_RenderAccTime += DeltaTime;

		if (m_RenderTime >= 0.15f)
		{
			m_RenderTime -= 0.15f;
			m_Render = !m_Render;
		}

		if (m_RenderAccTime >= 3.f)
		{
			m_RenderState = true;
			m_Render = true;
...
void CGameObject::PostUpdate(float DeltaTime)
{
	...
	if (m_Animation)
	{
		...
		//if (FrameData.Pivot == Vector2(-1.f, -1.f))
		//{
		//	m_Pivot = m_OriginPivot;
		//}

		//else if (FrameData.Pivot != Vector2(-1.f, -1.f))
		//{
...
void CGameObject::Render(HDC hDC, float DeltaTime)
{
	...
	if (m_Render)
	{
		if (m_Animation)
		{
			...
			// 프레임마다 정해준 피봇이 있다면 현재 피봇으로 설정
			// 피봇이 -1.f, -1.f 라면 정해준 피봇이 없으므로 생략
			if (FrameData.Pivot == Vector2(-1.f, -1.f))
			{
				m_Pivot = m_OriginPivot;
			}
