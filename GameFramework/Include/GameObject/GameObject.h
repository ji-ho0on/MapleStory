...
protected:
	...
	// 랜더링 시 깜빡거림 처리를 위한 랜더 변수
	bool		m_RenderState;
	bool		m_Render;
	float		m_RenderTime;
	float		m_RenderAccTime;
...
public:
	...
	const Vector2& GetOriginPivot()	const
	{
		return m_OriginPivot;
	}
...
public:
	...
	void SetOriginPivot(float x, float y)
	{
		m_OriginPivot.x = x;
		m_OriginPivot.y = y;
	}

	void SetOriginPivot(const Vector2& Pivot)
	{
		m_OriginPivot = Pivot;
	}
