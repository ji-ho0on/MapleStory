...
private:
	...
	Vector2	m_LookPos;					// 카메라의 목적지
	Vector2 m_PrevLookPos;				// 카메라의 이전 위치



	// 위치보정용 변수
	float	m_FollowTime;				// 카메라가 위치를 보정하는데 걸리는 시간
	float	m_ProportionalConst;		// 속도 비례 상수
	float	m_FollowAccTime;			// 카메라가 목적지까지 따라가는동안 누적된 시간

public:
	void SetLookPosition(const float& MoveDist)
	{
		m_ProportionalConst = -2.f * MoveDist / (m_FollowTime * m_FollowTime);
		m_FollowAccTime = 0.f;
	}

	void CalculateDiff(float DeltaTime);
